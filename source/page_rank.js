    ...
    var reduceIngoings = function (runk, id, callback) {
        client.zscore(HOST + "RANK", id, function (err, old_rank) {
            client.zcount(HOST + "OUT" + id, 0, '+inf', function (err, countout) {
                ...
                callback(null, runk + BETA * (old_rank / countout));
            });
        });
    };
...
//            console.log("final ranks" + new_ranks);
            // lets find a sum of all ranks
            var sum_rank = new_ranks.reduce(function(prev, current){ return prev + current;});
//            console.log("sum_rank: " + sum_rank);
//            console.log("delta = " + (BASE - sum_rank) / N);
...
//                    console.log("final ranks" + final_ranks);
...
//                                console.log(id, score, final_ranks[id-1]);
...
//                        console.log("run next iteration");
    ...
    var Results = function(){
        var top5 = [];
        client.zrevrange(HOST + "RANK", 0, 4, function(err, top){
            console.log(top);
...
//                console.log(EMAIL, finals, HOST, N);
                ...
                var email2     = new sendgrid.Email({
                    ...
                    subject:  "Report on PageRank weights for top 5 " + url.parse(HOST).hostname + " pages, to " + EMAIL,
                    text:     ''
                });
